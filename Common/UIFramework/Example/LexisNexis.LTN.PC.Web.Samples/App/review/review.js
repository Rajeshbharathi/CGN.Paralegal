...
///#source 1 1 /App/review/app.constants.js
(function () {
    'use strict';
    var ERRORS_LIST = {
        "Custom": "Custom",
        400: 'Bad Request',
        404: 'Page Not Found',
        901: 'Unauthorized service call'
    }
    angular.module('app').constant('ERRORS_LIST', ERRORS_LIST);

}());
///#source 1 1 /App/review/app.config.js
(function () {
    "use strict";
    function config($provide, WebApiClientServiceErrorDecoratorProvider) {
        ...
        WebApiClientServiceErrorDecoratorProvider.decorate($provide, ['WebApiClientService']);

    }
    angular.module('app').config(config);
    config.$inject = ['$provide', 'WebApiClientServiceErrorDecoratorProvider'];
}());

///#source 1 1 /App/review/review.config.js
'use strict';
var Configuration= {
    reviewer: {
        docSetNavigationAllowed: true,
        swapEnabled: true,
        zoomEnabled: true,
        highlightEnabled: true,
        featuresEnabled: true,
        splitterPanesConfig: [
            { collapsible: false, resizable:true,size: "70%", min: "50%", contentUrl: "../review/includes/LeftPaneView.html" },
            { collapsible: true, resizable: true, size: "30%", min: "15%", contentUrl: "../review/includes/RightPaneView.html" }
        ],
        featureTabTemplates: [
            { title: "Assisted Review", templateUrl: "../review/includes/AssistedReviewTabView.html" },
            { title: "Fields", templateUrl: "../review/includes/FieldTabView.html" }
        ]
    }
}
...
///#source 1 1 /App/common/decorators/WebApiClientServiceErrorDecorator.js
;(function () {
    'use strict';

    angular.module('app').provider('WebApiClientServiceErrorDecorator', function () {
        ...
        function decorate($injector, obj, func) {
            return angular.extend(function () {
                var handler = $injector.get('WebApiClientServiceErrorDecorator');
...
(function () {
    ...
    angular.module('app').factory('ErrorHandlerService', ['$log', 'ERRORS_LIST', function ($log, ERRORS_LIST) {
              ...
              buildErrorMessage: function (err, func) {
                  ...
                  if (err && !angular.isUndefined(err.status)) {
                      if (err.data && err.data.ExceptionType) {
                          var type = err.data.ExceptionType,
                         message = err.data.Message,
                         cause = err.data.ExceptionMessage;
                          err = type + ": " + message + "[ " + cause + " ]";
                      }

                      if (err.data && err.MessageDetail) {
                          err = ERRORS_LIST[err.status] + ":" + err.Message + "[" + err.MessageDetail +"]";
                      } else if (err.data && err.data.MessageDetail) {
                          err = ERRORS_LIST[err.status] + ":" + err.data.Message + "[" + err.data.MessageDetail + "]";
                      }

                      if (err.message) {
                          err = ERRORS_LIST[err.status]+ ":" + err.message;
                      }
...
(function () {
     ...
     * 
     * lnInputValidationSync - takes an object with custom validation methods which do not return a promise
     * eg: syncValidations ={
     ...
     *       }
     * 
     * lnInputValidationBatch - takes an array of custom validation methods [not async validations] which return a promise[eg. $http calls]   
...
(function () {
    ...
    function run() {
        //todo: don't cache while developing
        //note: this doesn't work well the UI bootstrap template cache!

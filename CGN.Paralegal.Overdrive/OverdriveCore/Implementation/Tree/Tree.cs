...
namespace PH.DataTree
{
   ...
   public class DTreeNode<T> 
   {
      ...
      {
         get { return mNodes ?? (mNodes = new DTreeNodeCollection<T>(this)); }
      }
      ...
      {
         get {
             return DepthFirstNodeEnumerator.Select(node => node.Value);
         }
      }
      ...
      public IEnumerable<DTreeNode<T>> DepthFirstNodeEnumerator
      {
         get
         {
            ...
            if (mNodes != null)
            {
               foreach (IEnumerator<DTreeNode<T>> childEnum in mNodes.Select(child => child.DepthFirstNodeEnumerator.GetEnumerator()))
               {
      ...
      {
         get {
             return BreadthFirstNodeEnumerator.Select(node => node.Value);
         }
      }
      #endregion
         ...
         : System.Collections.CollectionBase, IEnumerable<DTreeNode<T>>
   {
      ...
      {
          return InnerList.Cast<DTreeNode<T>>().GetEnumerator();
      }
